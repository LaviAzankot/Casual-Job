...
async function register(req, res) {
  const { name, email, password, phone, address, biography } = req.body;
  const profileImage = req.files.profileImage[0].filename;
  const profolioImages = req.files.profolioImages.map((file) => file.filename);
  ...
  try {
    ...
    // Create a new account
      ...
      const newUser = await db.query(
        "INSERT INTO users (email, password, name, phone, address, profileImage, biography, profolioImages) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id",
        [
          email,
          hashedPassword,
          name,
